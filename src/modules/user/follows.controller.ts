...
export class FollowController {
    ...
    async createFollow(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const follow = await this.followService.createFollow(follower_id, following_id);
    ...
    async unfollow(req: Request, res: Response, next: NextFunction) {
        try {
            const following_id = req.params.id;
            const follower_id = req.user?.id as string; 
            const result = await this.followService.unfollow(follower_id, following_id);
            res.status(204).send();
        } catch (error) {
            next(error)
        }
    ...
    async getFollowers(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const followers = await this.followService.getFollowers(user_id);
    ...
    async checkMutualFollow(req: Request, res: Response, next: NextFunction) {
        try {
            const following_id = req.params.id;
            const follower_id = req.user?.id as string;
            const is_mutual = await this.followService.checkMutual(follower_id, following_id);
